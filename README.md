...
## Prisma config
This project uses **Prisma v7**, which requires `prisma.config.ts` at the root (without it, `prisma migrate`/`generate`/`studio` won't find your `DATABASE_URL` correctly). It reads `DATABASE_URL` from `.env` via `dotenv/config`, so make sure `.env` exists before running any `prisma` CLI command.
